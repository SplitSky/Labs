...
# same charge same flavour for ptcone30
lepchargeCut = "lep_charge[0] * lep_charge[1] == 1 && lep_type[{0}]=={1} && lep_n==2 "
...
# same charge same flavour for etcone20
lepchargeCut = "lep_charge[0] * lep_charge[1] == -1 && lep_type[{0}]=={1} && lep_n==2 "
chargeCut = "(" + lepchargeCut.format(0, 13) + "&&" + lepchargeCut.format(1, 13) + ")"
...
### finding backgrounds - 23/02 -inv mass cut
inv_mass = "sqrt(2*lep_pt[0]*lep_pt[1]*(cosh(lep_eta[0]- lep_eta[1])- cos(lep_phi[0]-lep_phi[1])))"
...
### additional background cuts 23/02
inv_mass = "sqrt(2*lep_pt[0]*lep_pt[1]*(cosh(lep_eta[0]- lep_eta[1])- cos(lep_phi[0]-lep_phi[1])))"
t.SetAlias("inv_mass", inv_mass)
inv_massCut = " inv_mass >= 60e3 && inv_mass <= 120e3 "
lepchargeCut = " lep_charge[0] * lep_charge[1] == -1 && lep_n==2 && lep_type[{0}] == lep_type[{1}] && (lep_type[{0}]== 11 || lep_type[{0}]== 13 || lep_type[{0}]== 15)"

chargeCut = "(" + lepchargeCut.format(0, 1) + " && " + inv_massCut + ")"
t.Draw("inv_mass >> h_lep_pt_mean(100,0,140e3)", weighting + "*" + chargeCut)
h_lep_pt_mean = r.gDirectory.Get("h_lep_pt_mean")
h_lep_pt_mean.Write()
###
## pt cone and et cone for additional background cuts
inv_mass = "sqrt(2*lep_pt[0]*lep_pt[1]*(cosh(lep_eta[0]- lep_eta[1])- cos(lep_phi[0]-lep_phi[1])))"
t.SetAlias("inv_mass", inv_mass)
inv_massCut = " inv_mass >= 60e3 && inv_mass <= 120e3 "
lepchargeCut = " lep_charge[0] * lep_charge[1] == -1 && lep_n==2 && lep_type[{0}] == lep_type[{1}] && (lep_type[{0}]== 11 || lep_type[{0}]== 13 || lep_type[{0}]== 15)"
...
chargeCut = "(" + lepchargeCut.format(0, 1) + " && " + inv_massCut + ")"
t.Draw("lep_ptcone30 >> lep_ptcone30(100,0,140e3)", weighting + "*" + chargeCut)
lep_ptcone30 = r.gDirectory.Get("lep_ptcone30")
lep_ptcone30.Write()
...
chargeCut = "(" + lepchargeCut.format(0, 1) + " && " + inv_massCut + ")"
t.Draw("lep_etcone20 >> lep_etcone20(100,0,140e3)", weighting + "*" + chargeCut)
lep_etcone20 = r.gDirectory.Get("lep_etcone20")
lep_etcone20.Write()
...
hist1 = histFile1.Get("lep_etcone20")
hist2 = histFile2.Get("lep_etcone20")
hist3 = histFile3.Get("lep_etcone20")
hist4 = histFile4.Get("lep_etcone20")
hist5 = histFile5.Get("lep_etcone20")
hist6 = histFile6.Get("lep_etcone20")
hist7 = histFile7.Get("lep_etcone20")