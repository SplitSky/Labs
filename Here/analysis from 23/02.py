...
def Analyse(t, weighting):
    ...
    inv_massCut = " inv_mass >= 70e3 && inv_mass <= 120e3 "
    ptconeCut = " lep_ptcone30 < 3e3 "
    transverseCut = " lep_pt > 20e3  "
    lepchargeCut = " lep_charge[0] * lep_charge[1] == -1 && lep_n==2 && lep_type[{0}] == lep_type[{1}] && (lep_type[{0}]== 11 || lep_type[{0}]== 13 || lep_type[{0}]== 15)"
    # + ptconeCut + " && "
    chargeCut = "(" + lepchargeCut.format(0, 1) + " && " + inv_massCut + " && " + transverseCut + " )"
    ...
    # different charge same flavour cut
